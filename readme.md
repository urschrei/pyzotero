...
2. The following object methods are available:
    ...
    * `bib_items()`: returns a list containing HTML-formatted bibliography entries for each item. When calling this method, you may specify a `'style'` key, the value of which can be any valid Zotero Style Repository entry. Example: `'style': 'mla'`
